...
#include "perception_asr_stressoverflow/DetectedBagMonitor.hpp"
...
DetectedBagMonitor::DetectedBagMonitor()
: Node("bag_monitor"),
  tf_buffer_(),
  tf_listener_(tf_buffer_)
{
  marker_pub_ = create_publisher<visualization_msgs::msg::Marker>("bag_marker", 1);

  timer_ = create_wall_timer(
    100ms, std::bind(&DetectedBagMonitor::control_cycle, this));
}

void
DetectedBagMonitor::control_cycle()
{
  geometry_msgs::msg::TransformStamped robot2person;

  try {
    robot2person = tf_buffer_.lookupTransform(
      "base_link", "detected_bag", tf2::TimePointZero);
  ...
  RCLCPP_INFO(
    get_logger(), "Bag detected at (%lf m, %lf m, , %lf m) = %lf rads",
    x, y, z, theta);
